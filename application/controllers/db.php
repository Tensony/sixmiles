...
class Db extends CI_Controller {
	...
	*/
	...
	public function export_csv($db, $table)
	{
		// 1. Initialize database connection dynamically
		$this->dbmodel->initialize($db);
		
		// 2. Fetch all records from the table
		$query = $this->dbmodel->theDB->get($table);
		
		// 3. Load database utility with the dynamic database object
		$this->load->dbutil($this->dbmodel->theDB);
		
		// 4. Format as CSV
		$csv_data = $this->dbutil->csv_from_result($query);
		
		// 5. Force download the file
		$this->load->helper('download');
		force_download($table . '_' . date('Y-m-d') . '.csv', $csv_data);
	}

	public function export_sql($db, $table)
	{
		// 1. Initialize database connection dynamically
		$this->dbmodel->initialize($db);
		
		// 2. Load database utility with the dynamic database object
		$this->load->dbutil($this->dbmodel->theDB);
		
		// 3. Backup preferences (only for the requested table)
		$prefs = array(
			'tables'        => array($table),
			'ignore'        => array(),
			'format'        => 'txt', // txt format for raw SQL
		...
		// 4. Run the backup
		$sql_data = $this->dbutil->backup($prefs);
